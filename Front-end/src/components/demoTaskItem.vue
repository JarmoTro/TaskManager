<template>
  <div class="card card-body m-3 text-center" v-if="!isHidden">
    <div
      class="modal fade"
      v-bind:id="'taskDeletionModal_' + this.task.id"
      tabindex="-1"
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">
              Are you sure you want to delete the task {{ this.task.title }} ?
            </h5>
            <button
              type="button"
              class="btn-close"
    ...
    <div>
      <h3 class="m-3 d-block" v-bind:id="'taskTitle_' + this.task.id">
        {{ this.task.title }}
      </h3>
      <form
        v-on:submit.prevent="updateTask"
        class="me-3 d-none"
        v-bind:id="'taskEditForm_' + this.task.id"
      >
        <input
          type="text"
          name="title"
          class="form-control m-3"
          v-bind:id="'taskTitleUpdateInput_' + this.task.id"
        ...
        <input
          ...
          name="deadline"
          class="form-control m-3"
          v-bind:id="'taskDateUpdateInput_' + this.task.id"
          v-bind:value="this.task.deadline"
          required
        />
      </form>
      <h4 class="ms-3 d-block" v-bind:id="'taskDeadline_' + this.task.id">
        Due by {{ this.task.deadline.toDateString() }}
      </h4>
      ...
      <h4
        v-if="
          ...
          this.task.deadline.getTime() < new Date().getTime()
        "
      ...
      <div>
        <i
          ...
          v-bind:data-bs-target="'#taskDeletionModal_' + this.task.id"
        ></i>
        <i
          class="fa-solid fa-pencil fa-xl text-primary m-3"
          v-on:click="toggleTaskEditForm(this.task.id)"
          role="button"
        ></i>
      </div>
    </div>
    ...
    <button
      ...
      v-bind:data-bs-target="'#detailsArea_' + task.id"
    >
      View details
    </button>
    <div class="collapse text-center" v-bind:id="'detailsArea_' + task.id">
      ...
      >
        <div
          class="modal fade"
          v-bind:id="'subtaskDeletionModal_' + subtask.id"
          tabindex="-1"
        >
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                ...
                <button
                  ...
                  class="btn btn-danger"
                  data-bs-dismiss="modal"
                  v-on:click="deleteSubtask(subtask.id)"
                >
                  DELETE
                </button>
              </div>
        ...
        <div class="d-inline-block">
          ...
          >
            <input
              ...
              name="title"
              class="form-control m-3"
              v-bind:id="'subtaskUpdateInput_' + subtask.id"
          ...
          <h4 class="m-3" v-bind:id="'subtaskTitle_' + subtask.id">
            {{ subtask.title }}
          </h4>
          <button
            type="button"
            v-on:click="toggleSubtaskState(subtask.id)"
          ...
          <button
            ...
            v-bind:data-bs-target="'#subtaskDeletionModal_' + subtask.id"
            data-bs-toggle="modal"
          >
      ...
      <div
        ...
        role="button"
        v-bind:id="'subtaskCreateFormToggler_' + this.task.id"
      ...
      <div
        ...
        v-bind:id="'subtaskCreateForm_' + this.task.id"
      >
        <form v-on:submit.prevent="createSubtask">
          <input
            type="text"
            v-bind:id="'subtaskCreateInput_' + this.task.id"
            name="title"
  ...
  methods: {
    ...
    toggleTaskEditForm() {
      ...
      const formTitleInput = document.getElementById(
        "taskTitleUpdateInput_" + this.task.id
      );
      const formDateInput = document.getElementById(
        "taskDateUpdateInput_" + this.task.id
      );
    ...
    toggleSubtaskCreateForm() {
      ...
      const formToggler = document.getElementById(
        "subtaskCreateFormToggler_" + this.task.id
      );
      const formInput = document.getElementById(
        "subtaskCreateInput_" + this.task.id
      );
