<template>
  <div class="card card-body m-3" v-if="!isHidden">
    <div
      class="modal fade"
      v-bind:id="'taskDeletionModal_' + this.task._id"
      tabindex="-1"
    ...
    <div>
      <h3 class="m-3 d-block" v-bind:id="'taskTitle_' + this.task._id">
        {{ this.task.title }}
      </h3>
      <form
        v-on:submit.prevent="updateTask"
        class="me-3 d-none"
        v-bind:id="'taskEditForm_' + this.task._id"
      >
        <input
          type="text"
          name="title"
          class="form-control m-3"
          v-bind:id="'taskTitleUpdateInput_' + this.task._id"
          v-bind:value="this.task.title"
          required
          maxlength="100"
        />
        ...
        <input
          ...
          name="deadline"
          class="form-control m-3"
          v-bind:id="'taskDateUpdateInput_' + this.task._id"
          v-bind:value="this.task.deadline"
          required
        />
      </form>
      <h4 class="ms-3 d-block" v-bind:id="'taskDeadline_' + this.task._id">
        Due by {{ new Date(this.task.deadline).toDateString() }}
      </h4>
      <h4 v-if="completedTaskPercentage == 100" class="text-success ms-3">
        COMPLETED!
      </h4>
      <h4
        v-if="
          completedTaskPercentage != 100 &&
          new Date(this.task.deadline).getTime() < new Date().getTime()
        "
      ...
      <div>
        <i
          class="fa-solid fa-trash fa-xl text-danger m-3"
          role="button"
          data-bs-toggle="modal"
          v-bind:data-bs-target="'#taskDeletionModal_' + this.task._id"
        ></i>
        <i
          class="fa-solid fa-pencil fa-xl text-primary m-3"
          v-on:click="toggleTaskEditForm(this.task._id)"
          role="button"
        ></i>
      </div>
    </div>
    ...
    <button
      class="btn btn-primary btn-inline m-3"
      :class="{
        'bg-danger':
          new Date(this.task.deadline).getTime() < new Date().getTime() &&
          completedTaskPercentage != 100,
      ...
      v-bind:data-bs-target="'#detailsArea_' + task._id"
    >
      View details
    </button>
    <div class="collapse text-center" v-bind:id="'detailsArea_' + task._id">
      <div
        class="card card-body text-start m-3 text-light"
        :class="{
          'bg-success': subtask.finished,
          'bg-danger': !subtask.finished,
        }"
        v-for="subtask in this.task.subtasks"
      >
        <div
          class="modal fade"
          v-bind:id="'subtaskDeletionModal_' + subtask._id"
          tabindex="-1"
        >
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                ...
                <button
                  ...
                  class="btn btn-danger"
                  data-bs-dismiss="modal"
                  v-on:click="deleteSubtask(subtask._id)"
                >
                  DELETE
                </button>
              </div>
        ...
        <div class="d-inline-block">
          <form
            v-on:submit.prevent="updateSubtaskTitle"
            class="me-3 d-none"
          ...
          >
            <input
              ...
              name="title"
              class="form-control m-3"
              v-bind:id="'subtaskUpdateInput_' + subtask._id"
              v-bind:value="subtask.title"
              required
              maxlength="100"
            />
            <input type="hidden" name="subtaskId" v-bind:value="subtask._id" />
            <button type="submit" class="btn btn-primary d-inline-block m-3">
              SAVE CHANGES
            </button>
            <button
              type="button"
              class="btn btn-secondary d-inline-block m-3"
              v-on:click="toggleSubtaskEditForm(subtask._id, subtask.title)"
            >
              CANCEL
            </button>
          </form>
          <h4 class="m-3" v-bind:id="'subtaskTitle_' + subtask._id">
            {{ subtask.title }}
          </h4>
          <button
            type="button"
            v-on:click="toggleSubtaskState(subtask._id)"
            class="btn btn-outline-light m-3"
          >
            <i
              :class="{
                'fa-solid fa-xmark fa-2xl': !subtask.finished,
                'fa-solid fa-check fa-2xl': subtask.finished,
              }"
            ></i>
          </button>
          <button
          ...
          <button
            type="button"
            class="btn btn-outline-light m-3 d-inline xxl-d-block"
            v-bind:data-bs-target="'#subtaskDeletionModal_' + subtask._id"
            data-bs-toggle="modal"
          >
      ...
      <div
        class="card card-body m-3 d-block"
        role="button"
        v-bind:id="'subtaskCreateFormToggler_' + this.task._id"
        v-on:click="toggleSubtaskCreateForm"
      >
        <i class="fa-solid fa-plus fa-2xl m-3"></i>
      </div>
      <div
        class="card card-body m-3 d-none"
        v-bind:id="'subtaskCreateForm_' + this.task._id"
      >
        <form v-on:submit.prevent="createSubtask">
          <input
            type="text"
            v-bind:id="'subtaskCreateInput_' + this.task._id"
            name="title"
...
export default {
  ...
  methods: {
    getCompletedTaskPercentage() {
      ...
      this.completedTaskPercentage = Math.round(
        (completedTasks / this.task.subtasks.length) * 100
      );
    },
    ...
    toggleTaskEditForm() {
      ...
      const formTitleInput = document.getElementById(
        "taskTitleUpdateInput_" + this.task._id
      );
      const formDateInput = document.getElementById(
        "taskDateUpdateInput_" + this.task._id
      );
    ...
    updateTask(submitEvent) {
      ...
      const newDeadline = new Date(
        submitEvent.target.elements.deadline.value
      ).getTime();
    ...
    toggleSubtaskCreateForm() {
      const form = document.getElementById(
        "subtaskCreateForm_" + this.task._id
      );
      const formToggler = document.getElementById(
        "subtaskCreateFormToggler_" + this.task._id
      );
      const formInput = document.getElementById(
        "subtaskCreateInput_" + this.task._id
      );
